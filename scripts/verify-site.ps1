...
try {
    ...
    $initial = Invoke-Eval "(()=>({title:document.title,overview:Boolean(document.querySelector('.site-overview')),noIntroActions:!document.querySelector('#start-exploration')&&!document.querySelector('[data-list-start]'),noTagUi:!document.querySelector('#tag-filter')&&!document.querySelector('.panel-tags')&&!document.querySelector('[data-tag]'),genreLabel:document.querySelector('.zone-title h2').textContent.trim(),exploreLabel:document.querySelector('.panel-zone-title h2').textContent.trim(),empty:document.querySelector('#exploration-panel').dataset.state==='empty',noDialog:!document.querySelector('dialog'),noGenrePage:!document.querySelector('#genre-view'),nodes:document.querySelectorAll('.genre-node').length,errors:window.__verificationErrors}))()"

    $genreFlow = Invoke-Eval "(async()=>{const node=[...document.querySelectorAll('.genre-node')].find(x=>x.dataset.genre==='$Genre');node.click();for(let i=0;i<80&&document.querySelector('#exploration-panel').dataset.state!=='genre';i++)await new Promise(r=>setTimeout(r,80));const genreUrl=location.search;const samples=document.querySelectorAll('.specimen-card').length;document.querySelector('.specimen-card').click();for(let i=0;i<100&&document.querySelectorAll('#branch-options button').length<3;i++)await new Promise(r=>setTimeout(r,80));const itemUrl=location.search;const branches=[...document.querySelectorAll('#branch-options button')].map(x=>x.dataset.branchGenre+':'+x.dataset.branchItem);const source=document.querySelector('.source-button').href;const target=document.querySelector('.source-button').target;const recordDate=document.querySelector('.metadata dd').textContent.trim();const onlyRecordDate=document.querySelectorAll('.metadata div').length===1&&document.querySelector('.metadata dt').textContent.trim().length>0;const panel=document.querySelector('#exploration-panel');const panelOverscroll=getComputedStyle(panel).overscrollBehaviorY;const dock=document.querySelector('.item-explore-dock');const controlsAtTop=dock.compareDocumentPosition(document.querySelector('.item-media'))&Node.DOCUMENT_POSITION_FOLLOWING;const before=itemUrl;const next=document.querySelector('[data-next]:not(:disabled)');if(next){next.click();await new Promise(r=>setTimeout(r,80));history.back();for(let i=0;i<60&&location.search!==before;i++)await new Promise(r=>setTimeout(r,80));}const backRestored=location.search===before;const branch=document.querySelector('#branch-options button');const branchKey=branch.dataset.branchGenre+':'+branch.dataset.branchItem;branch.click();for(let i=0;i<80&&document.querySelector('#exploration-panel').dataset.genre+':'+document.querySelector('#exploration-panel').dataset.item!==branchKey;i++)await new Promise(r=>setTimeout(r,80));const stored=JSON.parse(sessionStorage.getItem('yohaku.exploration.v1'));return {genreUrl,itemUrl,samples,state:document.querySelector('#exploration-panel').dataset.state,branches:branches.length,uniqueBranches:new Set(branches).size,source,target,recordDate,onlyRecordDate,panelOverscroll,controlsAtTop:Boolean(controlsAtTop),trailHidden:!document.querySelector('.trail')&&!document.querySelector('[data-reset-exploration]'),backRestored,branchChanged:document.querySelector('#exploration-panel').dataset.genre+':'+document.querySelector('#exploration-panel').dataset.item===branchKey,stored:Boolean(stored&&stored.visited.length&&stored.trail.length),errors:window.__verificationErrors};})()"

    $similarFlow = Invoke-Eval "(async()=>{state.exploration={visited:[],trail:[]};saveExploration();await selectGenre('$Genre',{itemId:'$firstItemId',history:false});for(let i=0;i<100&&document.querySelectorAll('#branch-options button').length<3;i++)await new Promise(r=>setTimeout(r,80));const keys=['$Genre`:$firstItemId'];let immediateBacktrack=false,repeated=false;for(let step=0;step<12;step++){const button=document.querySelector('#branch-options button');if(!button)break;const targetKey=button.dataset.branchGenre+':'+button.dataset.branchItem;if(keys.length>1&&targetKey===keys[keys.length-2])immediateBacktrack=true;if(keys.includes(targetKey))repeated=true;button.click();for(let i=0;i<100;i++){const current=document.querySelector('#exploration-panel').dataset.genre+':'+document.querySelector('#exploration-panel').dataset.item;if(current===targetKey&&document.querySelectorAll('#branch-options button').length===3)break;await new Promise(r=>setTimeout(r,80));}keys.push(targetKey);}return {steps:keys.length-1,unique:new Set(keys).size,total:keys.length,immediateBacktrack,repeated,keys};})()"

    $controls = Invoke-Eval "(()=>{document.querySelector('[data-view=list]').click();const listVisible=!document.querySelector('#genre-list').hidden;const listCards=document.querySelectorAll('.genre-list-card').length;const listTags=document.querySelectorAll('.genre-list-card small').length;document.querySelector('[data-view=map]').click();return {listVisible,listCards,listTags,mapVisible:!document.querySelector('#genre-map').hidden,noTagUi:!document.querySelector('#tag-filter')&&!document.querySelector('.panel-tags')&&!document.querySelector('[data-tag]')};})()"
    ...
    Send-Cdp 'Page.navigate' @{ url = "$baseUrl`?genre=$Genre&item=$firstItemId" } | Out-Null
    Wait-For "document.querySelector('#exploration-panel').dataset.state==='item'"
    $mobile = Invoke-Eval "(()=>{const mapNode=document.querySelector('#genre-map');const stageNode=document.querySelector('#map-stage');const map=mapNode.getBoundingClientRect();const stage=stageNode.getBoundingClientRect();const panel=document.querySelector('#exploration-panel').getBoundingClientRect();const nodeRects=[...document.querySelectorAll('.genre-node')].map(x=>x.getBoundingClientRect());return {clientWidth:document.documentElement.clientWidth,scrollWidth:document.documentElement.scrollWidth,noOverflow:document.documentElement.scrollWidth<=document.documentElement.clientWidth,panelOverflow:getComputedStyle(document.querySelector('#exploration-panel')).overflowY,panelAfterMap:panel.top>=map.bottom-2,dockPosition:getComputedStyle(document.querySelector('.item-explore-dock')).position,branches:document.querySelectorAll('#branch-options button').length,touchAction:getComputedStyle(mapNode).touchAction,stageFits:stage.left>=map.left-1&&stage.right<=map.right+1&&stage.top>=map.top-1&&stage.bottom<=map.bottom+1,nodesFit:nodeRects.every(x=>x.left>=map.left-2&&x.right<=map.right+2&&x.top>=map.top-2&&x.bottom<=map.bottom+2),errors:window.__verificationErrors};})()"
    ...
    $failed = -not $initial.overview -or -not $initial.noIntroActions -or -not $initial.noTagUi -or [string]::IsNullOrWhiteSpace([string]$initial.genreLabel) -or [string]::IsNullOrWhiteSpace([string]$initial.exploreLabel) -or -not $initial.empty -or -not $initial.noDialog -or -not $initial.noGenrePage -or $initial.nodes -ne $publishedCount -or @($initial.errors).Count -ne 0
    $failed = $failed -or $genreFlow.genreUrl -ne "?genre=$Genre" -or -not $genreFlow.itemUrl.Contains("genre=$Genre&item=") -or $genreFlow.samples -lt 1 -or $genreFlow.state -ne 'item' -or $genreFlow.branches -ne 3 -or $genreFlow.uniqueBranches -ne 3 -or -not $genreFlow.source.StartsWith('http') -or $genreFlow.target -ne '_blank' -or [string]::IsNullOrWhiteSpace([string]$genreFlow.recordDate) -or -not $genreFlow.onlyRecordDate -or $genreFlow.panelOverscroll -ne 'contain' -or -not $genreFlow.controlsAtTop -or -not $genreFlow.trailHidden -or -not $genreFlow.backRestored -or -not $genreFlow.branchChanged -or -not $genreFlow.stored -or @($genreFlow.errors).Count -ne 0
    $failed = $failed -or $similarFlow.steps -ne 12 -or $similarFlow.unique -ne $similarFlow.total -or $similarFlow.immediateBacktrack -or $similarFlow.repeated
    $failed = $failed -or -not $controls.listVisible -or $controls.listCards -ne $publishedCount -or $controls.listTags -ne 0 -or -not $controls.mapVisible -or -not $controls.noTagUi
    ...
    $failed = $failed -or -not $mobile.noOverflow -or $mobile.panelOverflow -ne 'visible' -or -not $mobile.panelAfterMap -or $mobile.dockPosition -ne 'sticky' -or $mobile.branches -ne 3 -or $mobile.touchAction -ne 'pan-y' -or -not $mobile.stageFits -or -not $mobile.nodesFit -or @($mobile.errors).Count -ne 0
    ...
    [pscustomobject]@{ initial = $initial; genreFlow = $genreFlow; similarFlow = $similarFlow; controls = $controls; dateFormats = $dateFormats; layout = $layout; direct = $direct; legacy = $legacyResult; mobile = $mobile; motion = $motion } | ConvertTo-Json -Depth 8
