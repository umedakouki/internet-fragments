...
try {
    ...
    $failed = $failed -or $genreFlow.genreUrl -ne "?genre=$Genre" -or -not $genreFlow.itemUrl.Contains("genre=$Genre&item=") -or $genreFlow.samples -lt 1 -or $genreFlow.state -ne 'item' -or $genreFlow.branches -ne 3 -or $genreFlow.uniqueBranches -ne 3 -or -not $genreFlow.source.StartsWith('http') -or $genreFlow.target -ne '_blank' -or [string]::IsNullOrWhiteSpace([string]$genreFlow.recordDate) -or -not $genreFlow.onlyRecordDate -or -not $genreFlow.controlsAtTop -or -not $genreFlow.trailHidden -or -not $genreFlow.backRestored -or -not $genreFlow.branchChanged -or -not $genreFlow.stored -or @($genreFlow.errors).Count -ne 0
