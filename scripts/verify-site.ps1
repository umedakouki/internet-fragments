...
try {
    ...
    $map = Invoke-Eval "(()=>{const n=[...document.querySelectorAll('.genre-node')];const before=n.map(x=>({id:x.dataset.genre,left:x.style.left,top:x.style.top}));const tags=[...document.querySelectorAll('#tag-filter button')].slice(1);const tag=tags.find(b=>n.some(x=>!x.dataset.tags.split('|').includes(b.textContent)));if(tag)tag.click();const filtered=n.filter(x=>x.classList.contains('filtered')).length;n[0].click();const selected=Boolean(document.querySelector('.genre-node.selected'));document.querySelector('#random-genre').click();return {nodes:n.length,lines:document.querySelectorAll('.map-lines line').length,filtered,selected,preview:Boolean(document.querySelector('.open-selected')),positions:before,errors:window.__verificationErrors};})()"
    ...
    if ($KeepScreenshots) { Save-Screenshot $desktopShot }
