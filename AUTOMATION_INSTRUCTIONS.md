...
アーカイブを固定化させない。直近の日記と履歴を見て、同じ種類の主作業をなるべく連続させず、新設、深掘り、分割、統合、標本移動、非公開化を循環させる。新ジャンルは10〜50件の比較可能な資料で始める。既存ジャンルの追補・移動・再編に最低件数は設けない。

全ジャンルの共通タグ、件数の偏り、棚内の異質な `family`、新しく見つかった比較軸を毎回確認する。必要ならジャンル名、説明、関連、代表資料、地図配置も再編集する。サイトUIは、公開画面で不具合や明確な使いにくさを確認した場合だけ変更し、収集のたびに作り替えない。
...
公開せずローカル検証だけ行う場合は `-Publish` を外す。この処理は全ジャンル整合性、対象原典リンク、PC・390px実ブラウザー、URL・履歴・常時見える探索操作、記録日の統一表示、PagesのHTML・索引・ジャンルJSON・代表資産を確認する。
